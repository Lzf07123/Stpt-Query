...
| 前端 | Nginx 静态页面（原 dify-workflow-api 页面）+ 反向代理注入网关令牌 |
...
- 中期：前端按 Li-Design 模板做 Tailwind CSS 4 品牌化；Prometheus 指标告警；HTTPS 与密钥轮换
...
│   ├── app/main.py            # HTTP 层：/run /health* /service-status
...
│   ├── templates/default.conf.template  # 反代 /run 并注入令牌
│   ├── static/index.html      # 原 dify-workflow-api 完整页面
│   └── static/image/          # 站点图标
├── tests/                     # pytest（渲染/分类/编排/HTTP）
