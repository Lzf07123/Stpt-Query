...
**课表下载链接回看隐藏（2026-08-26 补充）**：回看历史时同样隐藏「点击下载课表（Word 文件）」
链接及其「此链接含个人令牌」提示（下载码短时有效）；实测历史回看 download/login/warn 均为
false、课表网格保留，实时结果不受影响。

**免密提示块剥离（2026-08-26 补充）**：回看历史与生成 PDF 不再显示「免密登录教务系统」
提示块（跳转码短时有效，回看/导出无意义）。后端新增 `strip_login_note` 并在 `md2pdf` 分支
生成 PDF 前剥离；前端保存历史与 `displaySavedRecord` 渲染时同样剥离（兼容旧记录）；
实测历史回看 `hasNote=false`、表格保留，实时结果仍含提示，后端新增用例 31/31 通过。

**期末成绩色阶修复（2026-08-26 补充）**：`.table-shell tbody td`（特异性更高）的默认前景色
覆盖了单类 `.score-high/mid/low/unpub` 的语义色；改为 `.markdown-body .table-shell
td.score-*` 恢复色阶（实测 95 绿 `rgb(42,124,82)`、82 棕黄 `rgb(154,92,5)`、
68 红 `rgb(196,55,55)`、未发布灰 `rgb(100,115,108)`；qwen3-vl-flash 复验一致）。
