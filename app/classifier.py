...
_RULES: Tuple[Tuple[str, str, List[str], List[str]], ...] = (
    ...
    (
        # 必须先于「账号或密码错误」：显式锁定/风控信号要求停止重试，
        # PASSERROR 只说明凭据被拒绝，不能与学校端限制混为一谈。
        "学校风控或账号临时锁定",
        r"USERLOCK|账号已被锁定|账号被锁定|密码错误次数过多|触发风控|风控",
        [
    ...
    (
        # PASSERROR 是 CAS 的密码错误信号；学校端拒绝凭据时上游也可能返回 401
        "账号或密码错误",
        r"PASSERROR|login verify failed|登录验证失败|账号或密码错误|账号不存在|教务系统登录失败",
